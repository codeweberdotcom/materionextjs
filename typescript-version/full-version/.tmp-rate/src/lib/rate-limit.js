...
const stores_1 = require("./rate-limit/stores");
const encodeStatesCursor = (payload) => {
    try {
...
const decodeStatesCursor = (cursor) => {
    if (!cursor) {
        return {};
    }
    try {
        ...
        if (parsed && typeof parsed === 'object') {
            return {
                ...
                lastEntry: parsed.lastEntry && typeof parsed.lastEntry === 'object'
                    ? {
                        id: typeof parsed.lastEntry.id === 'string' ? parsed.lastEntry.id : '',
                        source: parsed.lastEntry.source === 'manual' || parsed.lastEntry.source === 'state'
...
const getIpHashSecret = () => {
    ...
    if (!ipHashSecretWarningShown) {
        logger_1.default.warn('[rate-limit] RATE_LIMIT_IP_HASH_SECRET is not set; using a development fallback secret. Configure a strong secret in production.');
        ipHashSecretWarningShown = true;
    }
    return DEV_IP_HASH_SECRET;
};
const getEmailHashSecret = () => {
    const secret = process.env.RATE_LIMIT_EMAIL_HASH_SECRET;
    if (secret)
        return secret;
    if (!emailHashSecretWarningShown) {
        logger_1.default.warn('[rate-limit] RATE_LIMIT_EMAIL_HASH_SECRET is not set; using a development fallback secret. Configure a strong secret in production.');
...
const hashIpAddress = (ip) => {
    if (!ip)
        return null;
    const hmac = crypto_1.default.createHmac('sha256', getIpHashSecret());
...
const hashEmail = (email) => {
    if (!email)
        return null;
    const hmac = crypto_1.default.createHmac('sha256', getEmailHashSecret());
    hmac.update(`${EMAIL_HASH_VERSION}:${email.toLowerCase()}`);
    return hmac.digest('hex');
};
const extractIpPrefix = (ip) => {
    if (!ip)
        return null;
    if (ip.includes(':')) {
...
class RateLimitService {
    constructor() {
        ...
        this.configsLoadedAt = 0;
        this.configsLoading = null;
        this.configsReady = false;
        this.CONFIG_REFRESH_INTERVAL = 5 * 1000;
        this.missingConfigModules = new Set();
        this.fallbackConfigTemplate = {
            maxRequests: 100,
            windowMs: 60 * 1000,
            blockMs: 60 * 1000,
            warnThreshold: 5,
        ...
        this.store = (0, stores_1.createRateLimitStore)(prisma_1.prisma);
    ...
    async loadConfigs() {
        try {
            const configs = await prisma_1.prisma.rateLimitConfig.findMany();
            this.configs.clear();
            ...
            logger_1.default.error('Error loading rate limit configs:', { error: error, file: 'src/lib/rate-limit.ts' });
            this.setDefaultConfigs();
        }
        finally {
    ...
    async ensureConfigsFresh(force = false) {
        const now = Date.now();
    ...
    async recordEvent(params) {
        ...
        void events_1.eventService.record({
            source: 'rate_limit',
        ...
        try {
            await prisma_1.prisma.rateLimitEvent.create({
                data: {
                    module: params.module,
                    key: params.key,
                    userId: params.userId ?? null,
            ...
            const shouldRetryWithoutEmail = error instanceof client_1.Prisma.PrismaClientValidationError &&
            ...
            if (shouldRetryWithoutEmail) {
                try {
                    ...
                    logger_1.default.warn('RateLimitEvent table missing `email` column. Recorded event without email. Run the latest Prisma migrations to add it.');
                    return;
                }
                catch (retryError) {
                    logger_1.default.error('Error recording rate limit event (retry without email)', {
            ...
            logger_1.default.error('Error recording rate limit event', {
                error: error instanceof Error ? { name: error.name, message: error.message, stack: error.stack } : error
            });
        }
    }
    async ensureFallbackConfig(module) {
        const fallback = { ...this.fallbackConfigTemplate };
        fallback.isFallback = true;
        try {
            await prisma_1.prisma.rateLimitConfig.upsert({
                where: { module },
                update: {},
                create: {
                    module,
            ...
            logger_1.default.error('Failed to persist fallback rate limit config', {
                module,
    ...
    async checkLimit(key, module, options) {
        ...
        if (rawEmail) {
            ...
            if (emailHash)
                blockConditions.push({ emailHash });
        }
        if (options?.ipAddress) {
            ...
            if (ipHash)
                blockConditions.push({ ipHash });
            if (ipPrefix)
                blockConditions.push({ ipPrefix });
        }
        if (options?.keyType === 'ip') {
            ...
            if (keyIpHash)
                blockConditions.push({ ipHash: keyIpHash });
            if (keyIpPrefix)
                blockConditions.push({ ipPrefix: keyIpPrefix });
        }
        const activeBlock = await prisma_1.prisma.userBlock.findFirst({
            where: {
                OR: blockConditions,
        ...
        if (!config) {
            (0, rate_limit_1.incrementUnknownModuleMetric)(module);
            if (!this.missingConfigModules.has(module)) {
                this.missingConfigModules.add(module);
                logger_1.default.error('Rate limit config missing for module. Applying fallback monitor defaults.', { module });
            }
        ...
        const mode = config.mode === 'monitor' ? 'monitor' : 'enforce';
        const keyType = options?.keyType ?? (options?.userId ? 'user' : options?.ipAddress ? 'ip' : 'user');
    ...
    async getStats(module) {
        ...
        try {
            ...
            if (!config)
                return null;
            const activeStates = await prisma_1.prisma.rateLimitState.count({
                where: {
                    module,
                    blockedUntil: {
                        gt: new Date()
            ...
            const states = await prisma_1.prisma.rateLimitState.findMany({
                where: { module }
            });
            ...
            if (module === 'chat-messages') {
                ...
                totalRequests = await prisma_1.prisma.message.count({
                    where: {
            ...
            logger_1.default.error('Error getting rate limit stats:', { error: error, file: 'src/lib/rate-limit.ts' });
            return null;
        }
    }
    async updateConfig(module, config) {
        try {
            ...
            if (formattedSchemaError) {
                logger_1.default.error(formattedSchemaError.message, { cause: formattedSchemaError.cause });
                throw new Error(formattedSchemaError.message);
            }
            logger_1.default.error('Error updating rate limit config', {
                error: error instanceof Error ? { name: error.name, message: error.message, stack: error.stack } : error
            });
            throw error instanceof Error ? error : new Error('Failed to update rate limit config');
        }
    }
    describeSchemaMismatch(error) {
        if (error instanceof client_1.Prisma.PrismaClientValidationError) {
            const message = error.message || '';
            if (message.includes('Unknown argument `mode`')) {
                return {
                    message: 'RateLimitConfig is missing the `mode` column. Run `pnpm prisma migrate deploy` to apply the latest migrations.',
                    cause: message
                };
            }
            if (message.includes('Unknown argument `warnThreshold`')) {
                return {
                    message: 'RateLimitConfig is missing the `warnThreshold` column. Run `pnpm prisma migrate deploy` to update the schema.',
                    cause: message
                };
            }
            if (message.includes('Unknown argument `storeEmailInEvents`')) {
                return {
                    message: 'RateLimitConfig is missing the `storeEmailInEvents` column. Run `pnpm prisma migrate deploy` to apply the latest migrations.',
                    cause: message
                };
            }
            if (message.includes('Unknown argument `storeIpInEvents`')) {
                return {
                    message: 'RateLimitConfig is missing the `storeIpInEvents` column. Run `pnpm prisma migrate deploy` to apply the latest migrations.',
                    cause: message
                };
            }
            if (message.includes('Unknown argument `isFallback`')) {
                return {
                    message: 'RateLimitConfig is missing the `isFallback` column. Run `pnpm prisma migrate deploy` to apply the latest migrations.',
                    cause: message
                };
            }
        }
        if (error instanceof client_1.Prisma.PrismaClientKnownRequestError) {
            if (error.code === 'P2021' && typeof error.message === 'string' && error.message.includes('RateLimitEvent')) {
                return {
                    message: 'RateLimitEvent table is missing in the database. Run `pnpm prisma migrate deploy` to create it before using rate limit monitoring.',
                    cause: error.message
                };
            }
        }
    ...
    async resetLimits(key, module) {
        try {
            ...
            if (key)
                stateWhere.key = key;
            if (module)
                stateWhere.module = module;
            await prisma_1.prisma.rateLimitState.updateMany({
            ...
            if (key || module) {
                ...
                await prisma_1.prisma.userBlock.updateMany({
                    where: blockWhere,
                    data: {
                        isActive: false,
            ...
            logger_1.default.error('Error resetting rate limits:', { error: error, file: 'src/lib/rate-limit.ts' });
            return false;
        }
    }
    async clearState(stateId) {
        try {
            const state = await prisma_1.prisma.rateLimitState.findUnique({
                where: { id: stateId }
            });
            if (state) {
                await prisma_1.prisma.rateLimitState.delete({
                    where: { id: stateId }
                });
                await this.store.resetCache(state.key, state.module);
                return true;
            }
            const manualBlock = await prisma_1.prisma.userBlock.findUnique({
                where: { id: stateId }
            });
            if (!manualBlock) {
                return false;
            }
            await prisma_1.prisma.userBlock.update({
            ...
            logger_1.default.error('Error clearing rate limit state:', { error: error, file: 'src/lib/rate-limit.ts' });
            return false;
        }
    }
    async cleanupExpiredState(filter) {
        try {
            const now = new Date();
            const where = {
                AND: [
            ...
            await prisma_1.prisma.rateLimitState.deleteMany({ where });
        }
        catch (error) {
            logger_1.default.warn('Failed to cleanup expired rate limit states', {
                filter,
    ...
    async listStates(params = {}) {
        ...
        const [states, totalStates] = await Promise.all([
            prisma_1.prisma.rateLimitState.findMany({
                where,
                orderBy: [
                    { blockedUntil: 'desc' },
                    { updatedAt: 'desc' },
            ...
            prisma_1.prisma.rateLimitState.count({ where })
        ]);
        const manualBlockWhere = {
            isActive: true,
            ...(params.module
        ...
        const manualBlocks = await prisma_1.prisma.userBlock.findMany({
        ...
        const manualStateSet = uniqueManualTuples.length > 0
            ? new Set((await prisma_1.prisma.rateLimitState.findMany({
                where: {
        ...
        const manualOnlyBlocks = manualBlocks.filter(block => {
            const blockKey = block.userId || block.ipAddress || block.emailHash || block.email || block.id;
            if (!block.module || block.module === 'all')
                return true;
        ...
        if (keys.length) {
            const blockCounts = await prisma_1.prisma.rateLimitEvent.groupBy({
                by: ['key', 'module'],
                where: {
                    key: { in: keys },
        ...
        const adminIds = manualBlocks.reduce((acc, block) => {
            if (block.blockedBy)
                acc.push(block.blockedBy);
            return acc;
        }, []);
        ...
        const manualBlockEntries = manualBlocks
            .map(block => {
            const blockKey = block.userId || block.ipAddress || block.emailHash || block.email || block.id;
            if (!blockKey || !block.module)
                return null;
            if (block.blockedBy)
                adminIds.push(block.blockedBy);
            return {
        ...
        const blockKeyMap = new Map(manualBlockEntries.map(entry => [entry.key, entry.value]));
        for (const value of blockKeyMap.values()) {
            if (value.blockedBy)
                adminIds.push(value.blockedBy);
        }
        ...
        const latestBlockEvents = keys.length
            ? await prisma_1.prisma.rateLimitEvent.findMany({
                where: {
                    key: { in: keys },
        ...
        const stateItems = states.slice(0, limit).map(state => {
            ...
            const violationNumber = typeof totalViolations === 'number'
                ...
                : overLimit
                    ...
                    : latestBlockEvent
                        ...
                        : null;
            const reasonFromState = overLimit
            ...
            const reasonFromEvent = latestBlockEvent
                ? latestBlockEvent.module === 'chat-messages'
                    ? `Превышен лимит сообщений. Из разрешенных ${latestBlockEvent.maxRequests} за ${humanizeWindow(latestBlockEvent.windowEnd.getTime() - latestBlockEvent.windowStart.getTime())}, отправлено ${latestBlockEvent.count}.`
                    : `Превышен лимит: ${latestBlockEvent.count}/${latestBlockEvent.maxRequests} за ${humanizeWindow(latestBlockEvent.windowEnd.getTime() - latestBlockEvent.windowStart.getTime())}.`
                : null;
    ...
    async listEvents(params = {}) {
        ...
        const [events, total] = await Promise.all([
            prisma_1.prisma.rateLimitEvent.findMany({
                where,
                orderBy: { createdAt: 'desc' },
                take: limit + 1,
            ...
            prisma_1.prisma.rateLimitEvent.count({ where })
        ]);
        const userIds = Array.from(new Set(events.map(event => event.userId).filter((id) => Boolean(id))));
        ...
        const items = events.slice(0, limit).map(event => ({
            ...
            mode: (event.mode === 'monitor' ? 'monitor' : 'enforce'),
            count: event.count,
    ...
    async deleteEvent(eventId) {
        try {
            await prisma_1.prisma.rateLimitEvent.delete({
                where: { id: eventId }
            });
            return true;
        }
        catch (error) {
            logger_1.default.error('Error deleting rate limit event', {
                error: error instanceof Error ? { name: error.name, message: error.message, stack: error.stack } : error
            });
            return false;
        }
    ...
    async createManualBlock(params) {
        ...
        try {
            ...
            if (existingBlock) {
                ...
                const newUnblockedAt = params.durationMs && params.durationMs > 0 ? new Date(Date.now() + params.durationMs) : null;
                const updatedBlock = await prisma_1.prisma.userBlock.update({
            ...
            if (params.userId) {
                const userExists = await prisma_1.prisma.user.count({ where: { id: params.userId } });
                if (!userExists) {
            ...
            const block = await prisma_1.prisma.userBlock.create({
                data: {
                    module: params.module,
                    userId: params.userId ?? null,
                    email: storedEmail,
            ...
            logger_1.default.error('Error creating manual block', {
                error: error instanceof Error ? { name: error.name, message: error.message, stack: error.stack } : error
            });
            throw error;
        }
    }
    async deactivateManualBlock(blockId) {
        try {
            ...
            logger_1.default.error('Error deactivating manual block', {
                error: error instanceof Error ? { name: error.name, message: error.message, stack: error.stack } : error
            });
            return false;
        }
