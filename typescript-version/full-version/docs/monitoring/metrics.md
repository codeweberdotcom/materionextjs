...
- **Тестовые события:** удаляются автоматически через количество дней, указанное в `EVENT_RETENTION_TEST_EVENTS_DAYS` (по умолчанию 30). Очистка выполняется методом `EventRetentionService.cleanTestEvents()` и доступна через `/api/admin/events/retention?source=test_events`.
- **Реальные события:** TTL задаётся для каждого источника (`EVENT_RETENTION_<SOURCE>_DAYS`, по умолчанию 90). Очистка выполняется батчами (`EVENT_RETENTION_BATCH_SIZE`) через `EventRetentionService.cleanAll()`.
- **Статистика:** `GET /api/admin/events/retention` показывает текущие TTL, количество событий и сколько попадёт под очистку, включая блок `source: "test_events"`.
# 📊 Метрики (Prometheus)
