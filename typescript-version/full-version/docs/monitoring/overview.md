# 📊 Обзор мониторинга

## 🎯 Цели мониторинга

Система мониторинга обеспечивает:
- **Наблюдаемость** - понимание состояния системы
- **Быстрое обнаружение проблем** - алерты при сбоях
- **Анализ производительности** - метрики и трекинг
- **Отладка** - детальная информация об ошибках

## 🏗️ Архитектура

```
┌─────────────────┐
│   Приложение    │
│   (Next.js)     │
├─────────────────┤
│ Error Tracking  │  ← Sentry/GlitchTip
│   (Sentry)      │
├─────────────────┤
│   Метрики       │  ← Prometheus
│ (HTTP, WS, DB)  │
├─────────────────┤
│ Health Checks   │  ← /api/health
│                 │
├─────────────────┤
│ Application     │  ← Custom events
│   Insights      │
└─────────────────┘
```

## 🛠️ Компоненты

### **Error Tracking (Sentry)**
- ✅ Автоматический захват ошибок
- ✅ Source maps для продакшена
- ✅ Фильтрация чувствительных данных
- ✅ Группировка похожих ошибок

### **Метрики (Prometheus)**
- ✅ HTTP запросы (статус, время, роуты)
- ✅ WebSocket соединения
- ✅ База данных (запросы, время выполнения)
- ✅ Кастомные бизнес-метрики

### **Health Checks**
- ✅ Статус БД
- ✅ Статус внешних сервисов
- ✅ Общая работоспособность системы

### **Application Insights**
- ✅ Трекинг пользовательских событий
- ✅ Производительность операций
- ✅ Бизнес-метрики

## 📊 Доступ к данным

### **API эндпоинты**
```
GET /api/health      # Health check
GET /api/metrics     # Prometheus метрики
```

### **Внешние сервисы**
```
Sentry Dashboard    # Ошибки и производительность
Prometheus UI       # Метрики и графики
Grafana             # Дашборды (опционально)
```

### **Логи**
```
./logs/
├── application-*.log  # Основные логи
├── error-*.log        # Ошибки
└── *.log              # Другие логи
```

## 🚀 Быстрый старт

### **Проверка работоспособности**
```bash
# Health check
curl http://localhost:3000/api/health

# Метрики
curl http://localhost:3000/api/metrics
```

### **Тестирование error tracking**
```typescript
import { trackError } from '@/lib/sentry'

trackError(new Error('Test error'), { component: 'Test' })
```

### **Тестирование метрик**
```typescript
import { trackEvent, trackPerformance } from '@/lib/insights'

trackEvent('user_action', { action: 'click', page: 'dashboard' })
trackPerformance('api_call', 150, { endpoint: '/api/users' })
```

## 📈 Метрики и алерты

### **Ключевые метрики**
- **HTTP Response Time** - время ответа API
- **Error Rate** - процент ошибок
- **Database Query Time** - время выполнения запросов
- **WebSocket Connections** - активные соединения
- **Memory Usage** - использование памяти

### **Рекомендуемые алерты**
- HTTP 5xx ошибки > 5%
- Response time > 2 секунды
- Database timeouts
- Memory usage > 90%

## 🔧 Конфигурация

### **Переменные окружения**
```bash
# Error Tracking
SENTRY_DSN=https://your-dsn@sentry.io/project

# Метрики (опционально)
PROMETHEUS_PUSHGATEWAY_URL=http://localhost:9091

# Логи
LOG_LEVEL=info
```

### **Sentry конфигурация**
```typescript
// src/lib/sentry.ts
Sentry.init({
  dsn: process.env.SENTRY_DSN,
  environment: process.env.NODE_ENV,
  tracesSampleRate: 1.0,
  beforeSend(event) {
    // Фильтрация чувствительных данных
    return event
  }
})
```

### **Prometheus метрики**
```typescript
// src/lib/metrics.ts
export const httpRequestDuration = new promClient.Histogram({
  name: 'http_request_duration_seconds',
  help: 'Duration of HTTP requests',
  labelNames: ['method', 'route', 'status_code']
})
```

## 📱 Админ-панель

### **Страница мониторинга**
- **URL**: `/admin/monitoring`
- **Функции**:
  - Просмотр статуса системы
  - Тестирование error tracking
  - Информация о метриках
  - Ссылки на API

### **Доступ**
```typescript
// Только для администраторов
// Проверка в middleware или компоненте
```

## 🔄 Интеграция в код

### **Обработка ошибок**
```typescript
import { trackError } from '@/lib/sentry'

try {
  await riskyOperation()
} catch (error) {
  trackError(error, {
    component: 'UserService',
    operation: 'createUser',
    userId: user.id
  })
  throw error
}
```

### **Трекинг производительности**
```typescript
import { trackPerformance } from '@/lib/insights'

const start = Date.now()
const result = await apiCall()
trackPerformance('api_call', Date.now() - start, {
  endpoint: '/api/users',
  method: 'GET'
})
```

### **Трекинг событий**
```typescript
import { trackEvent } from '@/lib/insights'

trackEvent('user_login', {
  method: 'email',
  userId: user.id,
  timestamp: new Date().toISOString()
})
```

## 📊 Визуализация

### **Grafana Dashboards**
```json
// Пример панели
{
  "title": "HTTP Requests",
  "targets": [
    {
      "expr": "rate(http_requests_total[5m])",
      "legendFormat": "{{method}} {{route}}"
    }
  ]
}
```

### **Sentry Dashboard**
- **Issues** - список ошибок
- **Performance** - метрики производительности
- **Releases** - трекинг деплоев

## 🚨 Алерты и уведомления

### **Типы алертов**
- **Email** - критические ошибки
- **Slack** - оповещения команды
- **PagerDuty** - круглосуточная поддержка

### **Правила алертов**
```yaml
# Prometheus alerting rules
groups:
  - name: api_alerts
    rules:
      - alert: HighErrorRate
        expr: rate(http_requests_total{status=~"5.."}[5m]) > 0.05
        for: 5m
        labels:
          severity: critical
```

## 🔒 Безопасность

### **Фильтрация данных**
- Пароли и токены автоматически фильтруются
- PII данные маскируются
- Чувствительные заголовки удаляются

### **Доступ к метрикам**
- Метрики доступны только внутренне
- Sentry dashboard защищен
- Логи ротируются и архивируются

## 📈 Масштабирование

### **Многосерверная установка**
- Централизованный сбор метрик
- Распределенный трекинг ошибок
- Агрегация логов

### **Производительность**
- Асинхронная отправка метрик
- Буферизация логов
- Sampling для высоконагруженных систем

## 📚 Документация

- [Sentry Documentation](https://docs.sentry.io/)
- [Prometheus Documentation](https://prometheus.io/docs/)
- [Grafana Documentation](https://grafana.com/docs/)
- [Winston Logging](https://github.com/winstonjs/winston)