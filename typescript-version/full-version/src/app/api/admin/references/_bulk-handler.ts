...
export async function handleBulkOperation(
  ...
  try {
    ...
    const result = operationType === 'delete'
      ? await bulkOperationsService.bulkDeleteWithContext(ids, config, context, environment)
      : await bulkOperationsService.bulkUpdateWithContext(ids, {}, config, context, environment)
    ...
    const countKey = operationType === 'delete' ? 'deleted' : 'affected'

    return NextResponse.json({
      success: true,
      [countKey]: result.affectedCount,
