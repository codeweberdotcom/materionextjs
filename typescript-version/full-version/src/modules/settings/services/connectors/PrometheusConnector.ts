/**
 * Коннектор для Prometheus
 * Тестирует подключение через HTTP API
 * 
 * @module modules/settings/services/connectors/PrometheusConnector
 */
...
/**
 * Коннектор для Prometheus сервера
 */
export class PrometheusConnector extends BaseConnector {
  /**
   * Тестировать подключение к Prometheus через /-/healthy и /api/v1/status/buildinfo
   */
  async testConnection(): Promise<ConnectionTestResult> {
    ...
    try {
      ...
      const buildInfoResponse = await fetch(`${baseUrl}/api/v1/status/buildinfo`, {
        method: 'GET',
        headers,
        signal: AbortSignal.timeout(5000)
      })
      ...
      logger.error('[PrometheusConnector] Connection test failed', {
      ...
      return {
        success: false,
        latency,
        error: error instanceof Error ? error.message : 'Неизвестная ошибка подключения к Prometheus'
      }
    }
  }
